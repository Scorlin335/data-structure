...
class BSTree{
    ...
    public:
        ...
        void n_findValues(vector<T> &vec,T i,T j){
            stack<Node *> s;
            if (root_ == nullptr)
            {
                return;
        ...
        bool isChildTree(BSTree<T,Compare> &child){
            ...
            Node *curnode = root_;
            while (curnode != nullptr && curnode->data_ != child.root_->data_)
            {
        ...
        bool mirror02(){
            ...
            return mirror02(root_->left, root_->right_);
        }
    ...
    private:
        ...
        bool isChildTree(Node *node_1,Node *node_2){
            ...
            if(node_1->data_!=node_2->data_){
                return false;
            }
            return isChildTree(node_1->right_, node_2->right_);
        }
        //getLCA递归部分
        Node *getLCA(Node *node,T i, T j){
            if(node==nullptr){
                return node; // 如果到最后都没找到,那就是i和j有问题
            }
            if ((comp_(node->data_, i) && comp_(j, node->data_)) || (comp_(i, node->data_) && comp_(node->data_, j)))
            {
                return node;
            }
            else if (comp_(i, node->data_) && comp_(j, node->data_))
            {
            ...
            {
                return nullptr; //如果相等了那就是没找到,i和j有问题
            }
        }
        //镜像翻转递归部分
